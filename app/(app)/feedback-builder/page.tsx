...
export default function FeedbackBuilderPage() {
  const [activeTab, setActiveTab] = useState<'builder' | 'templates' | 'analytics' | 'preview' | 'responses'>('builder')
    ...
    <div className="p-4 sm:p-6 lg:p-8 space-y-6 max-w-7xl mx-auto">
      ...
      <Tabs value={activeTab} onValueChange={(v: any) => setActiveTab(v)} className="space-y-6">
        <TabsList className="grid grid-cols-5 bg-gray-100 p-1 rounded-xl max-w-3xl">
          ...
          <TabsTrigger value="responses" className="rounded-lg font-medium flex items-center gap-2">
            <Inbox className="w-4 h-4 text-orange-600" />
            Responses ({feedbackList.length})
          </TabsTrigger>
          ...
          {currentTemplate && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                ...
                {/* SHARE FEEDBACK LINK */}
                <Card className="shadow-sm border-emerald-200 bg-emerald-50/40 sticky top-[420px]">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription className="text-xs text-emerald-700">
                      Copy this link or share via WhatsApp to collect patient feedback.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div className="bg-white border border-emerald-200 rounded-lg p-2.5 text-xs font-mono text-gray-700 break-all select-all">
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        className="flex-1 text-xs font-semibold border-emerald-300 text-emerald-700 hover:bg-emerald-100"
                        onClick={() => {
                    ...
                    <Button
                      variant="outline"
                      size="sm"
                      className="w-full text-xs font-semibold border-green-400 text-green-700 hover:bg-green-50"
                      onClick={() => {
                        const link = typeof window !== 'undefined' ? `${window.location.origin}/feedback` : '/feedback'
                        const msg = `Hi! We'd love to hear your feedback about your recent visit. Please tap the link below:\n${link}\n\nThank you – PhysioNautics Team`
        ...
        {/* TAB: RECEIVED FEEDBACK / RESPONSES */}
        <TabsContent value="responses" className="space-y-4">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                <Inbox className="w-5 h-5 text-orange-500" />
                Received Patient Feedback
              </h2>
              <p className="text-xs text-gray-500">
                All feedback submissions from patients. Export to Excel for detailed analysis.
              </p>
            </div>
            <Button
          ...
          {feedbackList.length === 0 ? (
            <Card className="border-dashed border-2 border-gray-200 bg-gray-50/50">
              <CardContent className="p-12 text-center space-y-3">
                ...
                <p className="text-sm font-semibold text-gray-500">No feedback received yet</p>
                <p className="text-xs text-gray-400">Share the feedback link with patients to start collecting responses.</p>
              </CardContent>
            </Card>
          ) : (
            <div className="space-y-3">
              {feedbackList.map((fb: any, idx: number) => (
                <Card key={idx} className="shadow-sm border-gray-200 hover:shadow-md transition-shadow bg-white">
                  <CardContent className="p-5">
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                      <div className="space-y-2 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="font-bold text-sm text-gray-900">{fb.patient_name}</span>
                          {fb.patient_uid && (
                          ...
                          {fb.doctor_name && (
                            <Badge variant="outline" className="text-[10px] text-teal-800 border-teal-200 bg-teal-50 gap-1">
                              <Stethoscope className="w-3 h-3" />
                              Dr. {fb.doctor_name}
                            </Badge>
                          )}
                          {fb.centre_name && (
                            <Badge variant="outline" className="text-[10px] text-blue-700 border-blue-200 bg-blue-50 gap-1">
                      ...
                      <div className="flex sm:flex-col items-center sm:items-end gap-2 sm:gap-1.5 shrink-0">
                        <div className="flex items-center gap-1 bg-amber-50 border border-amber-200 rounded-lg px-3 py-1.5">
                          ...
                          <span className="text-sm font-bold text-amber-800">{fb.rating}/5</span>
                        </div>
                        <div className="grid grid-cols-3 gap-1 text-[10px]">
                          <div className="bg-teal-50 border border-teal-100 rounded px-1.5 py-0.5 text-center">
                            <span className="text-teal-700 font-semibold">{fb.treatment_rating || '-'}</span>
                            <span className="text-gray-400 ml-0.5">Rx</span>
                          </div>
                          <div className="bg-blue-50 border border-blue-100 rounded px-1.5 py-0.5 text-center">
                            <span className="text-blue-700 font-semibold">{fb.hygiene_rating || '-'}</span>
                            <span className="text-gray-400 ml-0.5">Hy</span>
                          </div>
                          <div className="bg-purple-50 border border-purple-100 rounded px-1.5 py-0.5 text-center">
                            <span className="text-purple-700 font-semibold">{fb.staff_rating || '-'}</span>
                            <span className="text-gray-400 ml-0.5">St</span>
                          </div>
                        </div>
                      </div>
                    </div>
