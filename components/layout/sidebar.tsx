...
import { LayoutDashboard, UserPlus, Users, Receipt, Stethoscope, Building2, UserCog, Tag, LogOut } from 'lucide-react'
...
function NavGroup({ title, items }: { title: string; items: typeof mainNav }) {
  ...
  return (
    <div className="space-y-1">
      <p className="px-3 text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">{title}</p>
      {items.map(item => {
  ...
  return (
    <aside className="w-64 h-screen bg-white border-r flex flex-col">
      <div className="flex items-center gap-3 px-4 py-5 border-b">
        ...
        <span className="text-lg font-bold text-gray-900">Physionautics</span>
      </div>
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-6">
        <NavGroup title="Main" items={mainNav} />
        <NavGroup title="Backend" items={backendNav} />
      </nav>
